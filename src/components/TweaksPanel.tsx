...
export function TweaksPanel({ tweaks, setTweaks }: { tweaks: Tweaks; setTweaks: Dispatch<SetStateAction<Tweaks>> }) {
  ...
  const update = (k: keyof Tweaks, v: string) => {
    const next = { ...tweaks, [k]: v } as Tweaks;
    setTweaks(next);
...
function Segmented({ opts, value, onChange }: { opts: Array<{ id: string; label: string }>; value: string; onChange: (value: string) => void }) {
